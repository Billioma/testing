...
import { useGetMakes } from "../../../services/admin/query/configurations";
...
export default function () {
  ...
  const { data, isLoading, refetch } = useGetMakes(
    {
  ...
  return (
    <Box w="full" border={"1px solid #E4E6E8"} borderRadius={"12px"}>
      <Flex justifyContent={"space-between"} alignItems="center" py={3} px={5}>
        <Text fontWeight="500">All Vehicle Makes</Text>
        <Flex gap="6px">
          <Button
          ...
          >
            <Text display={{ base: "none", md: "inline-flex" }}>Add make</Text>
            <FiPlus size={18} />
          </Button>
          <Button
            bg="white"
