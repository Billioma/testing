...
import { useCancelOpEv } from "../../../../services/operator/query/transactions";
...
const EvTableLayer = ({
  ...
  const openOption = (item, i) => {
    i === 1
      ? setShowCancel(true)
      : navigate(`/operator/transactions/event-parking/${item?.id} `);
  };
  ...
  const { mutate, isLoading: isCancel } = useCancelOpEv({
    onSuccess: () => {
      setShowCancel(false);
      successToast("Event Reservation Cancelled");
      refetch();
    },
    onError: (err) => {
      errorToast(
