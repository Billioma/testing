...
import DepartmentTableLayer from "../../../../components/data/Admin/Settings/DepartmentTableLayer";
...
import { useGetDepts } from "../../../../services/admin/query/settings";
...
export default function () {
  ...
  const { data, isLoading, refetch } = useGetDepts(
    {
    ...
    <Box border="1px solid #d4d6d8" borderRadius="8px" p="16px 23px 24px">
      <Filter
        ...
        main={
          <>
            <Button
              onClick={() => navigate(PRIVATE_PATHS.ADMIN_ADD_DEPT)}
              display="flex"
              bg="#000"
              gap="8px"
            >
              <Text fontSize="14px">Add New Department</Text>
              <MdAdd size="20px" />
            </Button>
            <Flex
